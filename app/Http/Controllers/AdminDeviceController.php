<?php namespace App\Http\Controllers;
	...
	class AdminDeviceController extends \crocodicstudio\crudbooster\controllers\CBController {
        ...
        public function Scan1()
		{
			...
			// ambil sessionid
			$device = DB::table('device')->select('name')->where('id_users', $id)->first();
			// cek device ready?
			...
			if($cek->status == "AUTHENTICATED"){
				...
				DB::table('device')->where('id_users', $id)->update(['status' => $cek->status ,'updated_at' => now()]);
				...
				if($res->error == "Session already exists"){
					...
					DB::table('device')->where('id_users', $id)->update(['name' => $newsessionID]);
					// dd($res);
				}
				$image = $res->qr;
